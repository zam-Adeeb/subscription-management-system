...
if (isset($_POST['add_books'])) {
  ...
  if (empty($bname)) {
    ...
    $add_book = mysqli_query($conn, "INSERT INTO book_info(`name`, `price`, `category`, `description`, `image`) VALUES('$bname','$price','$category','$desc','$img')") or die('Query failed');
    ...
    if ($add_book) {
      ...
      $message = 'Subscription Not Added Successfully';
    }
  }
}
...
if(isset($_POST['update_product'])){
  ...
  mysqli_query($conn, "UPDATE `book_info` SET name = '$update_name', description ='$update_description', price = '$update_price',category='$update_category' WHERE bid = '$update_p_id'") or die('query failed');
...
<a class="update_btn" style="position: fixed ; z-index:100;" href="total_books.php">See All Things</a>
  <div class="container_box">
    <form action="" method="POST" enctype="multipart/form-data">
      <h3>Add To <a href="index.php"><span>Subscription </span><span>System</span></a></h3>
      <input type="text" name="bname" placeholder="Enter Name" class="text_field ">
      <input type="number" min="0" name="price" class="text_field" placeholder="Enter price">
      <select name="Category" id="" required class="text_field">
            <option value="Adventure">Subscription</option>
            <option value="Magic">Movie</option>
            <option value="knowledge">TV series</option>
         </select>
      <textarea name="bdesc" placeholder="Enter description" id="" class="text_field" cols="18" rows="5"></textarea>
      <input type="file" name="image" accept="image/jpg, image/jpeg, image/png" class="text_field">
      <input type="submit" value="Add" name="add" class="btn text_field">
    </form>
  </div>
